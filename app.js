(() => {
  ...
  function driversManagementView() {
    ...
    return `<section class="drivers-hero"><div><small>DRIVER ADMINISTRATION</small><h2>Driver Control</h2><p>Manage drivers, app access, availability, live jobs and GPS from one screen.</p></div><a class="primary button-link" href="/driver.html" target="_blank" rel="noopener">Open Driver App ↗</a></section>
      ...
      <section class="driver-control-grid"><div class="panel"><div class="panelhead"><div><h2>Add a driver</h2><p>The login email must exactly match the Driver App email.</p></div></div><form id="driver-form" class="driver-admin-form"><label>Driver name<input name="name" required></label><label>Telephone<input name="phone"></label><label>Vehicle<input name="vehicle" placeholder="Luton Tail Lift"></label><label>Driver App login email<input name="login_email" type="email" required placeholder="driver@example.co.uk"></label><button class="primary">Add Driver & Link Login</button></form></div>
