...
function systemHealthSummary() {
  ...
  function bindApp() {
    ...
    document.getElementById('dispatch-bulk-priority')?.addEventListener('change', async event => { const priority=event.target.value; const ids=[...(state.dispatchSelectedJobs||[])]; if(!priority||!ids.length)return; const {error}=await db.from('jobs').update({priority,updated_at:new Date().toISOString()}).in('id',ids); if(error){showNotice(error.message,'error');render();return;} state.jobs.filter(j=>ids.includes(j.id)).forEach(j=>{j.priority=priority;j.updated_at=new Date().toISOString();}); showNotice(`${ids.length} job${ids.length===1?'':'s'} set to ${priority}.`,'ok'); state.dispatchSelectedJobs=[]; render(); });
