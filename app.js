(() => {
  ...
  const numberCode = () => {
    const usedNumbers = [...state.quotes, ...state.invoices]
      .map(item => item.quote_number || item.invoice_number || '')
      .map(value => String(value).match(/^KLS-111-(\d+)$/i))
      .filter(Boolean)
      .map(match => Number(match[1]));
    const nextNumber = (usedNumbers.length ? Math.max(...usedNumbers) : 0) + 1;
    return `KLS-111-${String(nextNumber).padStart(3, '0')}`;
  };
  ...
  function authView() {
    ...
    return `<div class="authwrap"><section class="authcard">
      ...
      <div class="authswitch"><a href="https://kingslogisticsservices.co.uk/privacy-notice/" target="_blank" rel="noopener">Privacy</a> · <a href="/driver-privacy.html" target="_blank" rel="noopener">Driver GPS notice</a></div>
  ...
  function driversManagementView() {
    ...
    const inviteFor = account => account ? `${location.origin}/driver.html?invite=${encodeURIComponent(String(account.email||'').trim().toLowerCase())}` : '';
    ...
    const rows = state.drivers.map(driver => {
      ...
      return `<article class="driver-control-card">
        ...
        <footer>${!account ? `<button class="secondary" data-link-driver="${driver.id}">Add login email</button>` : `<a class="primary button-link" href="mailto:${encodeURIComponent(account.email)}?subject=${encodeURIComponent('Your KLS Driver App login')}&body=${encodeURIComponent(`Hello ${driver.name||'Driver'},\n\nUse this personal link to create your KLS Driver App password:\n\n${inviteFor(account)}\n\nAfter confirming your email, sign in with ${account.email}.\n`)}">Send setup email</a><button class="secondary" data-copy-driver-invite="${driver.id}">Copy setup link</button>`}${account && String(account.email||'').trim().toLowerCase()===String(state.user?.email||'').trim().toLowerCase() ? `<button class="primary" data-repair-driver-login="${driver.id}">Relink this login</button>` : ''}${live ? `<a class="secondary button-link" target="_blank" href="https://www.google.com/maps?q=${current.last_latitude},${current.last_longitude}">Open GPS</a>` : ''}<a class="secondary button-link" href="/driver.html" target="_blank" rel="noopener">Open app</a><button class="secondary" data-edit-driver="${driver.id}">Edit</button><button class="danger" data-delete-driver="${driver.id}">Delete</button></footer>
      </article>`;
    }).join('');
...
function systemHealthSummary() {
  ...
  function settingsView() {
    ...
    const compliancePanel = panel('Privacy & compliance', `<div class="system-version"><div><small>CURRENT POLICY</small><b>1 August 2026</b><span>UK data protection and retention controls</span></div><span class="system-live">ACTIVE</span></div><div class="system-checks"><article class="system-check ok"><span>✓</span><div><b>Customer Privacy Notice</b><small>Explains customer, recipient, POD and contact-data use.</small></div></article><article class="system-check ok"><span>✓</span><div><b>Driver Privacy & GPS Notice</b><small>Limits tracking to active KLS work and records its purpose.</small></div></article><article class="system-check ok"><span>✓</span><div><b>Data Retention Policy</b><small>6 years essential records · 2 years photos/signatures · 90 days GPS.</small></div></article></div><div class="system-backup-actions"><a class="primary" href="https://kingslogisticsservices.co.uk/privacy-notice/" target="_blank" rel="noopener">Customer notice</a><a class="secondary" href="/driver-privacy.html" target="_blank" rel="noopener">Driver notice</a><a class="secondary" href="/retention-policy.html" target="_blank" rel="noopener">Retention policy</a></div><p class="system-help">Give customers access to the privacy notice and provide the driver notice before a driver uses the Driver App. Review these policies at least annually.</p>`, 'Published compliance information for Kings Logistics Services Ltd.');
    return `<section class="settings-layout"><div>${settingsPanel}</div><aside>${compliancePanel}${healthPanel}${exportPanel}</aside></section>`;
  }
  ...
  function bindApp() {
    ...
    document.querySelectorAll('[data-copy-driver-invite]').forEach(button => button.onclick = async () => {
      const driver=state.drivers.find(d=>d.id===button.dataset.copyDriverInvite);
      const account=state.driverAccounts.find(a=>a.driver_id===driver?.id&&a.active!==false);
      if(!driver||!account)return;
      const link=`${location.origin}/driver.html?invite=${encodeURIComponent(String(account.email||'').trim().toLowerCase())}`;
      try{await navigator.clipboard.writeText(link);showNotice(`${driver.name}’s setup link copied. You can paste it into WhatsApp or a text message.`,'ok');render();}catch(error){showNotice(`Copy this link: ${link}`,'error');render();}
    });
