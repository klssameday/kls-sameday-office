(() => {
  ...
  function dashboard() {
    ...
    const activity = state.jobs.map(j=>({
      ...
      text:j.job_status==='Delivered' ? `POD completed${j.pod_recipient_name ? ` for ${j.pod_recipient_name}` : ''}` : (j.assigned_driver_name ? `${j.assigned_driver_name} · ${j.customer_name||j.contact_name||'Customer'}` : `${j.customer_name||j.contact_name||'Customer'} · awaiting driver`),
      page:'jobs'
    ...
    return `<section class="command-hero"><div><small>KLS SAMEDAY OPERATIONS DASHBOARD</small><h2>${new Date().toLocaleDateString('en-GB',{weekday:'long',day:'numeric',month:'long'})}</h2><p>Live jobs, drivers, alerts and revenue from one control screen.</p></div><div><button class="secondary" data-action="refresh-dispatch">↻ Refresh</button><button class="primary" data-page="newquote">＋ New Job</button></div></section>
      <section class="command-kpis ops-kpis">${card('Jobs today',todayJobs.length,`${activeJobs.length} currently active`,'jobs')}${card('In progress',activeJobs.length,`${unassigned.length} unassigned`,'dispatch')}${card('Completed',completedJobs.length,`${completedJobs.filter(j=>String(j.collection_date||j.updated_at||'').slice(0,10)===today).length} today`,'jobs')}${card('Active drivers',onJobDrivers.length,`${availableDrivers.length} available`,'drivers')}${card('Today’s revenue',money(todayRevenue),`${todayJobs.length} scheduled`,'jobs')}${card('Needs attention',alerts.length,alerts.length?'Action required':'All clear','dashboard')}</section>
  ...
  function dispatchView() {
    ...
    const alertsHtml = alertRows.length ? alertRows.map(item=>`<button type="button" class="dispatch-alert-row ${item.level}" data-driver-open="${item.job.id}"><span>!</span><div><b>${esc(item.title)}</b><small>${esc(item.detail)}</small></div></button>`).join('') : '<div class="dispatch-all-clear"><span>✓</span><div><b>All clear</b><small>No unassigned or late collections.</small></div></div>';
  ...
  function fleetCentreView(){
    ...
    const days=(v)=>v?Math.ceil((new Date(v)-today)/86400000):9999;
    const vehicles=state.fleet||[]; const drivers=state.drivers||[];
    ...
    const alerts=[]; vehicles.forEach(v=>expiryFields.forEach(([label,key])=>{const n=days(v[key]);if(n<=30)alerts.push({vehicle:v,label,date:v[key],days:n});}));
    const openDefects=state.fleetDefects.filter(d=>d.status!=='Resolved');
    const tabs=['overview','drivers','vehicles','maintenance','defects','fuel'];
    const tabbar=`<div class="fleet-tabs">${tabs.map(t=>`<button class="${state.fleetTab===t?'active':''}" data-fleet-tab="${t}">${t[0].toUpperCase()+t.slice(1)}</button>`).join('')}</div>`;
    let body='';
    if(state.fleetTab==='drivers') body=`<div class="fleet-record-grid">${drivers.map(d=>`<article class="fleet-record"><div><small>DRIVER</small><h3>${esc(d.name||d.full_name||'Unnamed driver')}</h3></div><span class="status ${esc(String(d.status||d.availability_status||'Offline').toLowerCase().replace(/ /g,'-'))}">${esc(d.status||d.availability_status||'Offline')}</span><p>${esc(d.phone||'No phone saved')}</p><div class="fleet-detail-grid"><span>Licence <b>${fmtDate(d.licence_expiry)}</b></span><span>CPC <b>${fmtDate(d.cpc_expiry)}</b></span><span>Medical <b>${fmtDate(d.medical_expiry)}</b></span><span>Jobs <b>${state.jobs.filter(j=>j.assigned_driver_id===d.id||j.assigned_driver_name===d.name).length}</b></span></div></article>`).join('')||'<div class="empty">No drivers added yet.</div>'}</div>`;
    else if(state.fleetTab==='vehicles') body=`<div class="fleet-record-grid">${vehicles.map(v=>`<article class="fleet-record"><div><small>${esc(v.vehicle_type||'VEHICLE')}</small><h3>${esc(v.registration||v.reg||'No registration')}</h3><p>${esc([v.make,v.model].filter(Boolean).join(' ')||'Vehicle details')}</p></div><div class="fleet-detail-grid"><span>Mileage <b>${Number(v.current_mileage||v.mileage||0).toLocaleString('en-GB')}</b></span><span>Payload <b>${esc(v.payload||v.payload_kg||'—')}</b></span><span>MOT <b>${fmtDate(v.mot_expiry)}</b></span><span>LOLER <b>${fmtDate(v.loler_expiry)}</b></span></div></article>`).join('')||'<div class="empty">No vehicles added yet.</div>'}</div>`;
    else if(state.fleetTab==='maintenance') body=`<div class="fleet-alert-list">${alerts.sort((a,b)=>a.days-b.days).map(a=>`<article class="fleet-alert ${a.days<0?'overdue':'due'}"><div><b>${esc(a.vehicle.registration||a.vehicle.reg||'Vehicle')} — ${a.label}</b><small>${a.days<0?`${Math.abs(a.days)} days overdue`:`Due in ${a.days} days`}</small></div><span>${fmtDate(a.date)}</span></article>`).join('')||'<div class="empty">No maintenance or expiry alerts within 30 days.</div>'}</div>`;
    else if(state.fleetTab==='defects') body=`<button class="primary" data-new-defect>＋ Report Defect</button><div class="fleet-alert-list">${state.fleetDefects.map(d=>`<article class="fleet-alert ${d.severity==='Critical'?'overdue':'due'}"><div><b>${esc(d.vehicle)} — ${esc(d.category)}</b><small>${esc(d.notes||'No notes')} · ${fmtDate(d.date)}</small></div><span>${esc(d.status)}</span><button data-resolve-defect="${d.id}">Resolve</button></article>`).join('')||'<div class="empty">No defects reported.</div>'}</div>`;
    else if(state.fleetTab==='fuel') {const total=(state.fuelLogs||[]).reduce((s,x)=>s+Number(x.total_cost||x.cost||0),0);body=`<section class="crm-kpis">${card('Fuel entries',(state.fuelLogs||[]).length,'Recorded purchases')}${card('Fuel spend',money(total),'Total recorded')}${card('Average cost',(state.fuelLogs||[]).length?money(total/state.fuelLogs.length):money(0),'Per entry')}${card('Expenses',money((state.expenses||[]).reduce((s,x)=>s+Number(x.amount||0),0)),'Other fleet costs')}</section>`;}
    else body=`<section class="crm-kpis">${card('Drivers available',available.length,`${activeDrivers.length} active`)}${card('Vehicles',vehicles.length,'Fleet records')}${card('Maintenance alerts',alerts.length,alerts.length?'Needs attention':'All clear')}${card('Open defects',openDefects.length,openDefects.length?'Action required':'No open defects')}</section><div class="fleet-overview-grid">${panel('Upcoming maintenance',alerts.slice(0,6).map(a=>`<div class="mini-row"><b>${esc(a.vehicle.registration||a.vehicle.reg||'Vehicle')}</b><span>${a.label} · ${a.days<0?'Overdue':`${a.days} days`}</span></div>`).join('')||'<div class="empty">No upcoming alerts.</div>')}${panel('Driver status',drivers.slice(0,8).map(d=>`<div class="mini-row"><b>${esc(d.name||d.full_name||'Driver')}</b><span>${esc(d.status||d.availability_status||'Offline')}</span></div>`).join('')||'<div class="empty">No drivers added.</div>')}</div>`;
